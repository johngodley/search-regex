	...
	if ( error.code === 'rest_cookie_invalid_nonce' ) {
		return (
			<>
				...
				<p><ExternalLink url="https://searchregex.com/support/problems/cloudflare/">{ __( 'Read this REST API guide for more information.' ) }</ExternalLink></p>
			</>
		);
	}
	...
	if ( error.request && isSecurityPlugin( error.request.status, error.code ) ) {
		return (
			<>
				...
				<p><ExternalLink url="https://searchregex.com/support/problems/rest-api/">{ __( 'Read this REST API guide for more information.' ) }</ExternalLink></p>
			</>
		);
	}
	...
	if ( error.request && isServerError( error.request.status ) ) {
		return (
			<>
				...
				<p><ExternalLink url="https://searchregex.com/support/problems/rest-api/#http">{ __( 'Read this REST API guide for more information.' ) }</ExternalLink></p>
			</>
		);
	}
	...
	if ( message === 'failed to fetch' || message === 'not allowed to request resource' || message.indexOf( 'networkerror' ) !== -1 ) {
		return (
			<>
				...
				<p><ExternalLink url="https://searchregex.com/support/problems/rest-api/#url">{ __( 'Read this REST API guide for more information.' ) }</ExternalLink></p>
			</>
		);
	}
