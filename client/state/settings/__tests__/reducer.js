...
describe( 'groups reducer', () => {
	...
	test( 'SETTING_LOAD_SUCCESS sets load progress, values, and groups', () => {
		const state = reducer( DEFAULT_STATE, { type: SETTING_LOAD_SUCCESS, values: 1, groups: 2, installed: 'install', postTypes: [] } );

		expect( state ).toEqual( { ... DEFAULT_STATE, loadStatus: STATUS_COMPLETE, values: 1, groups: 2, postTypes: [], installed: 'install' } );
	} );
