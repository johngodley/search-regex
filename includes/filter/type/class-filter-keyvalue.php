...
class Filter_Keyvalue extends Filter_Type {
	 ...
	 */
	private string $key_logic = 'any';

	/**
	 * Logic to use for value
	 */
	private string $value_logic = 'any';
	 ...
	 * Key value flags
	 */
	private Search\Flags $key_flags;
	 ...
	 * Value flags
	 */
	private Search\Flags $value_flags;

	/**
	 * Key join
	 */
	private ?Sql\Join\Meta $join_key = null;

	/**
	 * Value join
	 */
	private ?Sql\Join\Meta $join_value = null;
	...
	public function __construct( array $item, Schema\Column $schema ) {
		...
		$this->key_flags = new Search\Flags( $item['keyFlags'] ?? [ 'case' ] );
		$this->value_flags = new Search\Flags( $item['valueFlags'] ?? [ 'case' ] );
	}
	...
	public function get_column_data( $column, $value, Source\Source $source, Action\Action $action ) {
		$meta_contexts = [];
		$raw_values = array_values( array_filter( explode( ',', $value ), fn( $v ) => $v !== '' ) );
